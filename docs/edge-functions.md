...
`microsoft-oauth-redirect`, `slack-oauth-redirect`, `clickup-oauth-redirect`, `zoho-oauth-redirect`),
...
| `clickup-oauth-start` | Begins the ClickUp connection. Requires the caller's JWT; 503 when `CLICKUP_CLIENT_ID` is unset. ClickUp has no scopes — the grant covers every workspace the user ticks on the consent screen. Reuses `google_oauth_states` |
| `clickup-oauth-redirect` | Redirect target from ClickUp — `verify_jwt = false`, the single-use `state` row authenticates it. Exchanges the code (`POST /api/v2/oauth/token`, query-string params), reads the authorised workspaces (`GET /api/v2/team`), seals the never-expiring token into `clickup_connections`. **Clears the saved channel when its workspace is no longer in the grant** |
...
| `manage-clickup` | ClickUp connection actions for the signed-in user: `status`, `channels`, `set_channel`, `disconnect`. `verify_jwt = true`, user token only. `channels` lists `type = CHANNEL` Chat channels across every authorised workspace (DMs dropped) via `GET /api/v3/workspaces/{id}/chat/channels`; `set_channel` re-lists and matches the id, storing the workspace alongside. `disconnect` deletes the row — ClickUp has no revoke endpoint |
